...
async function fetchCardData() {
  try {
    const res = await fetch(`www.upbeat.io.kr/api/cards/${cardId}/show`, {
      headers: {
...
async function handleStrategySelect(strategyText, buttonEl) {
  try {
    const res = await fetch(`www.upbeat.io.kr/api/cards/${cardId}/strategy`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${token}`
...
document.getElementById("submitBtn").addEventListener("click", async () => {
  ...
  try {
    const res = await fetch(`www.upbeat.io.kr/api/cards/${cardId}/answer`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${token}`
...
async function toggleLike(answerId, btnEl) {
  ...
  const url = liked
    ? `www.upbeat.io.kr/cards/${cardId}/likeCancel/${answerId}`
    : `www.upbeat.io.kr/cards/${cardId}/like`;
